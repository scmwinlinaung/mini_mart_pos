...
class DatabaseService {
  ...
  // Generic query method - use positional parameters for simple queries
  Future<Result> execute(String sql, [List<dynamic>? parameters]) async {
    final conn = await connection;
    if (parameters != null && parameters.isNotEmpty) {
      // For older postgres package compatibility - convert parameters one by one
      var finalSql = sql;
      for (int i = 0; i < parameters.length; i++) {
        finalSql = finalSql.replaceFirst(RegExp(r'\$\d+'), '@param$i');
      }
      final Map<String, dynamic> namedParams = {};
      for (int i = 0; i < parameters.length; i++) {
        namedParams['param$i'] = parameters[i];
      }
      return await conn.execute(Sql.named(finalSql), parameters: namedParams);
    } else {
